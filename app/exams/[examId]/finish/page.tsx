...
export default function ExamFinishPage() {
  ...
  const scoreDetails = session?.scoringDetails || [
    ...
    { label: 'Speaking', max: 30, achieved: 'pending', icon: <Award size={18} /> },
    { label: 'Writing', max: 30, achieved: 'pending', icon: <BarChart size={18} /> },
  ];
      ...
      <div className="relative z-10 w-full max-w-lg bg-white rounded-[40px] shadow-2xl shadow-blue-200/50 overflow-hidden mx-6 mt-10">
        ...
        <div className="p-10 pt-4">
          ...
          <div className="bg-blue-50/50 border-2 border-blue-100 rounded-3xl p-6 flex flex-col items-center mb-8 relative overflow-hidden">
            ...
            <span className="text-blue-500 font-bold uppercase tracking-wider text-xs mb-2 relative z-10">Estimated Score</span>
            <div className="text-5xl font-black text-blue-600 relative z-10">
               {totalScore}
            </div>
            <span className="text-slate-400 text-xs mt-2 relative z-10">/ 120 Total</span>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 mb-10">
            {scoreDetails.map((s: any, idx: number) => {
               ...
               return (
                <div key={idx} className="bg-slate-50 rounded-2xl p-4 border border-slate-100 flex flex-col relative overflow-hidden">
                  ...
                  <div className="flex items-baseline gap-1 relative z-10 p-1">
                    {isPending ? (
                      <span className="text-sm font-bold text-orange-400">Under Review</span>
                    ) : (
                      <>
                        <span className="text-2xl font-black text-slate-800">{s.achieved}</span>
                        <span className="text-xs font-bold text-slate-400">/ {s.max}</span>
                      </>
                    )}
                  </div>
                </div>
