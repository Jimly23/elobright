  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        ...
        const writingSection = sections.find((s: any) => s.title.toLowerCase().includes('writing')) || sections[0];
      ...
      <main className="relative z-10 flex-1 flex flex-col items-center justify-center p-6">
        <div className="w-full max-w-4xl bg-white rounded-[32px] shadow-2xl shadow-blue-200/40 p-8 md:p-16 border border-slate-50">
          ...
          <div className="flex justify-center">
            ...
            >
              {submitting ? 'Submitting...' : (currentQuestionIndex < questions.length - 1 ? 'Next Question' : 'Finish')}
            </button>
          </div>
        </div>
      </main>
